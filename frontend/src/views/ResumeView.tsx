      ...
      <div className="flex-1 flex flex-col gap-6 md:gap-8">
        ...
        >
          ...
          <div className="relative z-10 flex flex-col items-center text-center">
            ...
            <button 
              ...
              className="mt-6 md:mt-8 px-6 md:px-8 h-12 md:h-auto md:py-3 bg-white border border-indigo-100 rounded-full text-sm font-bold text-indigo-600 shadow-lg shadow-indigo-500/10 hover:bg-indigo-50 hover:shadow-indigo-500/20 transition-all flex items-center justify-center disabled:opacity-50"
            >
        ...
        <div className="bg-white/60 backdrop-blur-2xl rounded-3xl border border-white/40 shadow-xl shadow-indigo-500/5 p-6 md:p-8">
          ...
          <div className="space-y-3 md:space-y-4">
            {resumes.length > 0 ? (
              resumes.map((resume) => (
                <div key={resume.id} className="flex items-center justify-between p-3 md:p-4 rounded-2xl border border-white/60 bg-white/40 hover:bg-indigo-50/50 hover:border-indigo-200 transition-all group shadow-sm">
                  <div className="flex items-center gap-3 md:gap-4">
                    ...
                    <div className="min-w-0">
                      <p className="text-sm md:text-base font-bold text-slate-700 group-hover:text-indigo-700 transition-colors truncate max-w-[150px] md:max-w-none">{resume.fileName}</p>
                      <p className="text-[10px] md:text-xs text-slate-400 font-medium mt-0.5">
                        {new Date(resume.createdAt).toLocaleDateString()} • {(resume.fileSize / 1024 / 1024).toFixed(1)} MB
                      </p>
                    </div>
                  </div>
                  <button 
                    onClick={() => setSelectedReportId(resume.id)}
