...
const DocumentLibraryView: React.FC<DocumentLibraryViewProps> = ({ isPreviewMode = false, onPreviewModeChange }) => {
  ...
  const handleUpload = (file: File, category: string) => {
    const newDoc: Document = {
      ...
      title: file.name,
      category: (category === 'cover_letter' || category === 'other') ? 'resume' : category as DocType, // Map to existing types for mock
          ...
          {filteredDocs.length > 0 ? (
            filteredDocs.map((doc) => (
              ...
              >
                ...
                {/* Mobile Action Bar Fallback (if we wanted distinct mobile UI, but the above absolute positioning works well with hidden overflow on mobile if we adjust) 
                    Actually, for mobile, let's make it static at the bottom of the card instead of absolute hover.
                */}
